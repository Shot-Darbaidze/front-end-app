    ...
    <div className="relative bg-[#0F172A] pt-24 pb-10 sm:pt-32 sm:pb-16 px-4 sm:px-6 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto z-10">
        <div className="text-center max-w-2xl mx-auto mb-10">
            ...
            {isKa ? "იპოვე შენზე მორგებული " : "Find Your Perfect "}
            <span className="text-[#F03D3D]">{isKa ? "ინსტრუქტორი" : "Instructor"}</span>
            </h1>
            <p className="text-gray-400 text-base sm:text-lg">
            {isKa
              ? "შეადარე ფასები, შეფასებები და მიმოხილვები, რათა შენს საჭიროებებზე მორგებული ინსტრუქტორი აირჩიო."
        ...
        <div className="bg-white p-2 rounded-2xl shadow-2xl shadow-black/20 max-w-2xl mx-auto flex flex-col md:flex-row gap-2 transform hover:scale-[1.01] transition-transform duration-300">
          <div className="flex-1 relative group">
            ...
            <input
              type="text"
              placeholder={isKa ? "მოძებნე სახელით, სპეციალიზაციით..." : "Search by name, specialty..."}
              value={value}
            ...
            {value && (
              <button
                ...
                aria-label={isKa ? "ძიების გასუფთავება" : "Clear search"}
              >
                <X className="w-5 h-5" />
              </button>
            )}
